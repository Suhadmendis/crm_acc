...
if ($_GET["Command"] == "getdt") {
    ...
    if ($_GET["ls"] == "new") {
        $sql = "select * from msvessel order by id   limit 50";
    } else {
    ...
    while ($row = mysqli_fetch_array($result)) {
        $tb .= "<tr>";
        $tb .= "<td onclick=\"getcode('" . $row['id'] . "')\">" . $row['id'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['id'] . "')\">" . $row['vessel'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['id'] . "')\">" . $row['imo'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['id'] . "')\">" . $row['flagg'] . "</td>";
        $tb .= "</tr>";
    }
    $tb .= "</table>";
