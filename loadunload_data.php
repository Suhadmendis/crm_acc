...
if ($_GET["Command"] == "getdt") {
    ...
    if ($_GET["ls"] == "new") {
        $sql = "select * from MasterLoadUload    limit 50";
    } else {
    ...
    while ($row = mysqli_fetch_array($result)) {
        $tb .= "<tr>";
        $tb .= "<td onclick=\"getcode('" . $row['loadid'] . "')\">" . $row['loadid'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['loadid'] . "')\">" . $row['loaddesc'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['loadid'] . "')\">" . $row['active'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['loadid'] . "')\">" . $row['desc2'] . "</td>";
        $tb .= "</tr>";
    }
    $tb .= "</table>";
