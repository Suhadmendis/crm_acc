...
if ($_GET["Command"] == "getdt") {
	...
	if ($_GET["ls"] == "new") {
		$sql = "select * from MasterAirPort order by id desc limit 50";
	} else {
	...
	while ($row = mysqli_fetch_array($result)) {
		$tb .= "<tr>";
		$tb .= "<td onclick=\"getcode('" . $row['CODE'] . "')\">" . $row['CODE'] . "</td>";
		$tb .= "<td onclick=\"getcode('" . $row['CODE'] . "')\">" . $row['PORT'] . "</td>";
		$tb .= "<td onclick=\"getcode('" . $row['CODE'] . "')\">" . $row['COUNTRY'] . "</td>";
		$tb .= "<td onclick=\"getcode('" . $row['CODE'] . "')\">" . $row['TOWN'] . "</td>";
		$tb .= "<td onclick=\"getcode('" . $row['CODE'] . "')\">" . $row['OTHER'] . "</td>";
