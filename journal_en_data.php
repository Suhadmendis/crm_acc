...
if ($_GET["Command"] == "add_tmp") {
 ...
 $sql = "select C_CODE from lcodes where C_CODE = '" . $_GET['itemCode'] . "'";
    ...
    $sql = "Insert into tmp_che_data (code, descript,CN1,CN2,CN3, amount, tmp_no,form_no,descript1)values
			('" . $_GET['itemCode'] . "', '" . $_GET['itemDesc'] . "', '" . $_GET['CN1'] . "', '" . $_GET['CN2'] . "', '" . $_GET['CN3'] . "', " . $_GET['itemPrice'] . ",'" . $_GET['tmpno'] . "','" . $_GET['form'] . "','" . $_GET['txt_gl_name1'] . "') ";
    ...
    $ResponseXML .= "<sales_table><![CDATA[<table class=\"table\">
      <tr   class='info'>
                        ...
                        <th class=\"info\"style=\"width: 120px;\">DUMMY 1</th>
                        <th class=\"info\"style=\"width: 120px;\">DUMMY 2</th>
                        <th class=\"info\"style=\"width: 120px;\">DUMMY 3</th>
                        <th class=\"info\" style=\"width: 10px;\"></th>
    ...
    foreach ($conn->query($sql) as $row) {
        $ResponseXML .= "<tr>
                         ...
                         <td>" . $row['CN3'] . "</td>
                         <td></td>
                         <td onClick=\"cal('" .$row['amount'] . "','" .$_GET['form'] . "')\">" . number_format($row['amount'], 2, ".", ",") . "</td>
...
if ($_GET["Command"] == "save_item") {
    ...
    try {
        ...
        foreach ($conn->query($sql) as $row) {
            ...
            $sql = "insert into ledtrans (REFNO, BDATE, CODE, AMOUNT, FLAG, NARA,Currency,rate,curamo,CN1,CN2,CN3) value
                   ('" . $invno . "','" . $_GET["crndate"] . "','" . $row['code'] . "','" . $amo . "','CRE','" . $remarks . "','" . $_GET["cur"] . "','" . $_GET["Rate"] . "','" . $row['amount'] . "', '" . $_GET['CN1'] . "', '" . $_GET['CN2'] . "', '" . $_GET['CN3'] . "')";
            $conn->exec($sql);
