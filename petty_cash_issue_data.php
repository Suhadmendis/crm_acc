...
if ($_GET["Command"] == "save_item") {
    ...
    try {
        ...
        $remarks = $_GET['remark'];
        $reason = $_GET['reason'];
       $sql = "Insert into ledmas (REFNO, BDATE, type, DETAILS,reason, Currency,rate,tmp_no,b_ref) values
        ('" . $invno . "', '" . $_GET["crndate"] . "','PCI','" . $remarks . "','" . $reason . "','" . $_GET["cur"] . "', '" . $_GET["Rate"] . "', '" . $_GET['tmpno'] . "', '" . $_GET['iou_ref'] . "') ";
        $conn->exec($sql);
...
if ($_GET["Command"] == "pass_rec") {
    ...
    if ($row = $result->fetch()) {
        ...
        $ResponseXML .= "<b_ref><![CDATA[" . $row["b_ref"] . "]]></b_ref>";
