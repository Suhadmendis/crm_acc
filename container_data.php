...
if ($_GET["Command"] == "getdt") {
    ...
    if ($_GET["ls"] == "new") {
        $sql = "select * from MasterContainer order by id desc limit 50";
    } else {
    ...
    while ($row = mysqli_fetch_array($result)) {
        $tb .= "<tr>";
        $tb .= "<td onclick=\"getcode('" . $row['ConID'] . "')\">" . $row['ConID'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['ConID'] . "')\">" . $row['ConDes'] . "</td>";
        $tb .= "<td onclick=\"getcode('" . $row['ConID'] . "')\">" . $row['ConRef'] . "</td>";
        $tb .= "</tr>";
    }
    $tb .= "</table>";
